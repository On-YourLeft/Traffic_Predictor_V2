...
def run_mlops_pipeline():
    ...
    if not os.path.exists(data_path):
        raise FileNotFoundError(
            f"Cannot find {data_path}. Run notebooks/data_exploration.ipynb first!"
        )
    ...
    with mlflow.start_run(run_name="RF_Delhi_Fused_Production"):
        ...
        mlflow_sklearn.log_model(
            sk_model=model, artifact_path="fused_random_forest", signature=signature
        )
...
if __name__ == "__main__":
    ...
    if not os.path.exists("data"):
        print(
            "⚠️ Please run this script from the root 'traffic_predictor_v2' directory!"
        )
    else:
        run_mlops_pipeline()
